...
void loop() {
  ...
  len = Serial.readBytesUntil('#', (char *) bufferw1, 20) ; 
  // pad with spaces
  for (byte i = len; i < 20; i++) bufferw1[i] = ' ';     
  ...
  // Write block
  status = mfrc522.MIFARE_Write(block, &bufferw1[16], 16);
  if (status != MFRC522::STATUS_OK) {
    Serial.print(F("MIFARE_Write() failed: "));
    Serial.println(mfrc522.GetStatusCodeName(status));
    return;
  ...
  len = Serial.readBytesUntil('#', (char *) bufferw2, 20) ; 
  for (byte i = len; i < 20; i++) bufferw2[i] = ' ';     
  ...
  len = Serial.readBytesUntil('#', (char *) bufferw3, 30) ; 
  for (byte i = len; i < 30; i++) bufferw3[i] = ' ';     
  ...
  // Write block
  status = mfrc522.MIFARE_Write(block, &bufferw3[16], 16);
  if (status != MFRC522::STATUS_OK) {
    Serial.print(F("MIFARE_Write() failed: "));
    Serial.println(mfrc522.GetStatusCodeName(status));
    return;
